...
function [opts] = setEnvironment(type)
    ...
    if (isempty(type))
        ...
        switch type
            case 'super_speedy' % fastest, low resolution
            ...
            case 'speedy' % fast, high resolution
                opts.kde.Nkernels = 5000;
                opts.kde.learn_bw = false;
                opts.approximate_PMI = true;
                opts.num_scales = 1;
            ...
            case 'accurate' % slow, high resolution, high quality results
                opts.approximate_PMI = true;
                opts.PMI_predictor.Ntrees = 32;
                
            case 'super_accurate' % very slow, probably not worth the running time (about the same quality as 'accurate' mode)
                % no changes from the default options above
