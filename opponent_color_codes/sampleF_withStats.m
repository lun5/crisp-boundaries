...
function [F, pairStats] = sampleF_withStats(f_maps,Nsamples,opts)
    ...
    F = orderAB(F);
    dists = ((p1(:,1)-p2(:,1)).^2 + (p1(:,2)-p2(:,2)).^2).^0.5;
    pairStats = [min(dists), max(dists), mean(dists), std(dists), prctile(dists,95)];
end